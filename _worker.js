	...
	async fetch(request, env) {
		...
		if (!([mytoken, fakeToken, 访客订阅].includes(token) || url.pathname == ("/" + mytoken) || url.pathname.includes("/" + mytoken + "?"))) {
			...
			if (订阅格式 == 'base64' || token == fakeToken) {
				...
				subConverterUrl = `${subProtocol}://${subConverter}/sub?target=clash&url=${encodeURIComponent(订阅转换URL)}&insert=false&config=${encodeURIComponent(subConfig)}&emoji=true&list=false&tfo=true&scv=true&fdn=false&sort=false&new_name=true&udp=true&xudp=true&skip-cert-verify=true`;
